...
├── README.md
├── scripts/
│   ├── vms-create.sh                    # Multipass VM 4台を起動（入口）
│   ├── vms-delete.sh                    # Multipass VM を削除（入口）
│   ├── controller-ansible-run.sh        # es-controller 上で ansible-playbook（入口）
│   └── support/                         # 直接実行しない・障害時のみ
│       ├── common.sh
│       ├── ansible-inventory-generate.sh
│       ├── multipass-daemon-fix.sh
│       └── wsl-exe-launch-fix.sh
├── cloud-init/
